...
function createFrequencyList(head) {
  ...
  let current = head;
  while (current) {
    elemCount[current.val] = (elemCount[current.val] || 0) + 1;
    current = current.next;
  }

  let returnListCurrent = list;

  for (const count of Object.values(elemCount)) {
    returnListCurrent.next = new ListNode(count);
    returnListCurrent = returnListCurrent.next;
  }

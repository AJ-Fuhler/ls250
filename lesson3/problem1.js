...
function findPeakInTerrain(terrain) {
  if (terrain.length === 1) return terrain[0];
  let left = 0;
  let right = terrain.length - 1;

  while (left <= right) {
    if (left === 0 && terrain[left] > terrain[left + 1]) return left;
    if (right === terrain.length - 1 && terrain[right] > terrain[right - 1]) return right;
    const mid = Math.floor((left + right) / 2);
    const currentHeight = terrain[mid];
    const heightToLeft = mid > 0 ? terrain[mid - 1] : -Infinity;
    const heightToRight = mid < terrain.length - 1 ? terrain[mid + 1] : -Infinity;

    if (currentHeight > heightToLeft && currentHeight > heightToRight) {
      return mid;
    } else if (currentHeight < heightToLeft) {
      right = mid - 1;
    } else {
      left = mid + 1;
    }
